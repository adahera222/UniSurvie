        ...
        foreach (var so in Data.StoredObjectNames)
        {
            ...
            else
            {
                ...
                if (so.Components != null && so.Components.Count > 0)
                {
                    ...
                    {
                        if (!so.Components.ContainsKey(comp.GetType().AssemblyQualifiedName) && !so.Components.ContainsKey(comp.GetType().FullName))
                        {
                            Destroy(comp);
                        }
                    }
                            ...
                            try
                            {
                                                                                                    ...
                                                                                                    for (var i = 0; i < nlist.Count; i++)
                                                                                                    {
                                                                                                        if (JSONLevelSerializer.CustomSerializers.ContainsKey(tp))
                                                                                                        {
                                                                                                            JSONLevelSerializer.CustomSerializers[tp].Deserialize(
                                                                                                                Encoding.Default.GetBytes(UnitySerializer.UnEscape(comp.List[i].Data)), nlist[i]);
                                                                                                        }
                                                                                                        else
                                                                                                        {
                                ...
                                else
                                {
                                    ...
                                    for (var i = 0; i < list.Count; i++)
                                    {
                                        ...
                                        {
                                            JSONLevelSerializer.CustomSerializers[type].Deserialize(Encoding.Default.GetBytes(cp.List[i].Data), list[i]);
                                        }
                                        else
                                        {
